...
def is_abba(s):
    for i in range(len(s) - 3):
        x = s[i:i+4]
        if x[0] != x[1] and x[0] == x[3] and x[1] == x[2]:
            return True
    return False

def not_abba(s): return not is_abba(s)

def has_aba(outside, inside):
    for s in outside:
    ...
    for line in data.splitlines():
        ...
        def parse(s):
            ...
            if s3:
                parse(s3)
        parse(line)
        if any(map(is_abba, outside)) and all(map(not_abba, inside)):
            ans1 += 1
        if has_aba(outside, inside):
            ans2 += 1
    print(ans1)
    print(ans2)
