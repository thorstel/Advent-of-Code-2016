...
def solve(data):
    ...
    for line in data.splitlines():
        m = re.match(r'bot (\d+) gives low to (.*) and high to (.*)', line)
        if m:
            id = int(m[1])
            rules[id] = (m[2], m[3])
        else:
            s = line.split()
            val, bot = int(s[1]), int(s[5])
            bots[bot].append(val)

    def process_bot(id):
        assert len(bots[id]) < 3
        if len(bots[id]) < 2:
            return False
        # Part 1
        if 61 in bots[id] and 17 in bots[id]:
            print(id)
        low, high = min(bots[id]), max(bots[id])
        bots[id].clear()

        low_type, low_id = rules[id][0].split()
        low_id = int(low_id)
        if low_type == 'output':
            outputs[low_id] = low
        elif low_type == 'bot':
        ...
        high_type, high_id = rules[id][1].split()
        high_id = int(high_id)
        if high_type == 'output':
